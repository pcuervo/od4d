...
if(isset($pinnacle['topbar_height']) && isset($pinnacle['topbar']) && $pinnacle['topbar'] == 1) {
  $topbar_height = '#topbar {height:'.$pinnacle['topbar_height'].'px;} #topbar ul.sf-menu li a, #topbar .top-menu-cart-btn, #topbar .top-menu-search-btn, #topbar .topbarsociallinks li a, #topbar .nav-trigger-case .kad-navbtn, #topbar .kadence_social_widget {line-height:'.$pinnacle['topbar_height'].'px;}';
} else {
  $topbar_height = '';
} 
...
$htoppadding = '.trans-header #pageheader {padding-top:'.kadence_header_height().'px;}';

//Typography
if(!empty($pinnacle['font_h1'])) {
  $font_family = '.headerfont, .tp-caption, .yith-wcan-list li, .yith-wcan .yith-wcan-reset-navigation, ul.yith-wcan-label li a, .product_item .price {font-family:'.$pinnacle['font_h1']['font-family'].';} .topbarmenu ul li {font-family:'.$pinnacle['font_primary_menu']['font-family'].';} #kadbreadcrumbs {font-family:'.$pinnacle['font_p']['font-family'].';}';
} else {
  $font_family = '';
}  
...
if(!empty($pinnacle['th_header_border_color'])) {
  $header_border_color = '.headerclass {border-color:'.$pinnacle['th_header_border_color'].';}';
} else {
  $header_border_color = '';
}
// Sitewide Call
...
if(isset($pinnacle['page_max_width']) && $pinnacle['page_max_width'] == '1') {
    $page_max_width = '@media (min-width: 1200px) {.container {width: 970px;} aside.col-lg-3 {width: 33.33333333333333%;} .main.col-lg-9 {width: 66.66666666666666%;}#nav-main .sf-menu > .kt-lgmenu > ul {width:940px;}}';
    } else {
      $page_max_width = '';
    } 
  //topbar mobile
if(isset($pinnacle['topbar_mobile_hide']) && $pinnacle['topbar_mobile_hide'] == '1') {
    $topbar_mobile_hide = '@media (max-width: 767px) {#topbar{display:none; height: 0px !important;}}';
    } else {
      $topbar_mobile_hide = '';
    } 
...
if(isset($pinnacle['pagetitle_align']) && $pinnacle['pagetitle_align'] == 'right') {
  ...
  $topbar_layout = '.kad-topbar-left, .kad-topbar-left .topbar_social {float:right;}#topbar #mobile-nav-trigger, #topbar .kad-topbar-right ul.topbmenu, .kad-topbar-right .kad-topsearch-button, .kad-topbar-right .top-menu-cart-btn {float:left;} #topbar .kadence_social_widget {text-align: right;}.kad-topbar-right #kad-top-search-popup, .kad-topbar-right #kad-top-cart-popup{right: auto;left: 0px;}';
  } else {
  $topbar_layout = '';
  } 
...
if(isset($pinnacle['pinnacle_animate_in']) && $pinnacle['pinnacle_animate_in'] == 0) {
...
$kad_custom_css = '<style type="text/css">'.$topbar_height.$topbar_iconmenu_fontsize.$header_border_color.$htoppadding.$header_overlay_background_transparency.$font_family.$color_primary30.$color_grayfont.$color_footerfont.$posttitle.$pageheader_color.$home_action_padding.$sitewide_action_padding.$pagetitle_align_right.$color_primary.$pagetitle_align.$show_posttag.$x2logo.$x2logo_white.$ptitle_minheight.$animate.$topbar_layout.$topbar_mobile_hide.$hbg_color.$scrolling_background.$show_author.$show_postedin.$show_comment.$page_max_width.$show_date.$mobile_color
.$custom_css.'</style>';
