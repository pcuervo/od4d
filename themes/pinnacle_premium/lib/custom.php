...
function kad_panels_row_background_styles($fields) {
  ...
  $fields['padding_left'] = array(
        'name'      => __('Padding Left', 'pinnacle'),
        'type'      => 'measurement',
        'group'     => 'layout',
        'priority'  => 9,
      );
  $fields['padding_right'] = array(
        'name'      => __('Padding Right', 'pinnacle'),
        'type'      => 'measurement',
        'group'     => 'layout',
        'priority'  => 9,
      );
  ...
  $fields['background_image_style'] = array(
        'name'      => __('Background Image Style', 'pinnacle'),
        'type'      => 'select',
        'group'     => 'design',
        'default'   => 'center top',
        'priority'  => 6,
        'options'   => array(
             "cover"      => __("Cover", "pinnacle"),
             "parallax"   => __("Parallax", "pinnacle"),
             "no-repeat"  => __("No Repeat", "pinnacle"),
             "repeat"     => __("Repeat", "pinnacle"),
             "repeat-x"   => __("Repeat-X", "pinnacle"),
             "repeat-y"   => __("Repeat-y", "pinnacle"),
              ),
        );
  $fields['border_top'] = array(
        'name'      => __('Border Top Size', 'pinnacle'),
        'type'      => 'measurement',
        'group'     => 'design',
        'priority'  => 8,
  );
  $fields['border_top_color'] = array(
        'name'      => __('Border Top Color', 'pinnacle'),
        'type'      => 'color',
        'group'     => 'design',
        'priority'  => 8.5,
      );
  $fields['border_bottom'] = array(
        'name'      => __('Border Bottom Size', 'pinnacle'),
        'type'      => 'measurement',
        'group'     => 'design',
        'priority'  => 9,
  );
  $fields['border_bottom_color'] = array(
        'name' => __('Border Bottom Color', 'pinnacle'),
        'type' => 'color',
        'group' => 'design',
        'priority' => 9.5,
  );
...
add_action('init', 'pinnacle_qtranslate_support');
function pinnacle_qtranslate_support() {
...
function kad_lightbox_off() {
  global $pinnacle; 
  if(isset($pinnacle['kadence_lightbox']) && $pinnacle['kadence_lightbox'] == 1 ) {
...
function kad_lightbox_text() {
  global $pinnacle; if(!empty($pinnacle['lightbox_loading_text'])) {$loading_text = $pinnacle['lightbox_loading_text'];} else {$loading_text = 'Loading...';}
  if(!empty($pinnacle['lightbox_of_text'])) {$of_text = $pinnacle['lightbox_of_text'];} else {$of_text = 'of';}
  if(!empty($pinnacle['lightbox_error_text'])) {$error_text = $pinnacle['lightbox_error_text'];} else {$error_text = 'The Image could not be loaded.';}
...
function kt_get_default_settings_cyclone() {
    ...
    $defaults['license_id'] = '37';
    $defaults['license_key'] = 'e9EET1gH97gRcPOLc8ci1yz0qaINywgT';
...
function pinnacle_content($limit) {
