(function( $ ) {
    ...
    redux.field_objects.kad_slides = redux.field_objects.kad_slides || {};

    $( document ).ready(
        function() {
            //redux.field_objects.kad_slides.init();
        }
    );
    ...
    redux.field_objects.kad_slides.init = function( selector ) {
        ...
        if ( !selector ) {
            ...
            //selector = $( document ).find( ".redux-group-tab:visible" ).find( '.redux-container-kad_slides:visible' );
        ...
        $( selector ).each(
            function() {
                ...
                if ( parent.hasClass( 'redux-container-kad_slides' ) ) {
                ...
                el.find( '.redux-slides-remove' ).live(
                    'click', function() {
                        ...
                        if ( slideCount > 1 ) {
                            ...
                            $( this ).parents( '.redux-container-kad_slides:first' ).find( '.redux-slides-accordion-group:last' ).find( '.redux-slides-header' ).text( content_new_title );
                        }
                    }
                );

                //el.find( '.kad_redux-slides-add' ).click(
                el.find( '.kad_redux-slides-add' ).off('click').click(
                    function() {
