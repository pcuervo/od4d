...
// Don't duplicate me!
    if ( ! class_exists( 'ReduxFramework_extension_advanced_customizer' ) ) {
         ...
         * @since       1.0.0
         */
        class ReduxFramework_extension_advanced_customizer {
            ...
            public function __construct( $parent ) {
                ...
                add_filter( 'redux/customizer/panel/class_name', array( $this, 'panel_class' ) );
                add_filter( 'redux/customizer/section/class_name', array( $this, 'section_class' ) );
            }

            function panel_class() {
                return 'Redux_Advanced_Customizer_Panel';
            }

            function section_class() {
                return 'Redux_Advanced_Customizer_Section';
            }

            function create_field_classes( $option ) {
                if ( empty( $this->redux_fields ) ) {
                    $file_paths = glob( ReduxFramework::$_dir . 'inc/fields/*' );
                    foreach ( $file_paths as $file ) {
                        if ( $file != "section" && $file != "divide" && $file != "editor" ) {
                            $this->redux_fields[] = str_replace( ReduxFramework::$_dir . 'inc/fields/', '', $file );
                        }
                    }
                }
                $class_name = 'Redux_Customizer_Control_' . $option['type'];
                if ( ! class_exists( $class_name ) && ( in_array( $option['type'], $this->redux_fields ) || ( isset( $option['customizer_enabled'] ) && $option['customizer_enabled'] ) ) ) {

                    if ( ! file_exists( $this->upload_dir . $option['type'] . '.php' ) ) {
                        ...
                        $this->parent->filesystem->execute( 'put_contents', $this->upload_dir . $option['type'] . '.php', array( 'content' => $template ) );
                    }
                    if ( file_exists( $this->upload_dir . $option['type'] . '.php' ) ) {
                        include_once( $this->upload_dir . $option['type'] . '.php' );
                    }
                }
            }
            ...
            function enqueue_controls_css() {
                ...
                wp_enqueue_script(
                    'redux-extension-advanced-customizer',
                    $this->_extension_url . 'extension_advanced_customizer.js',
                    array( 'jquery', 'redux-js' ),
                    ReduxFramework_extension_advanced_customizer::$version . '4',
                    true
                );
            ...
            public function _register_customizer_controls( $wp_customize ) {
                if ( ! class_exists( 'Redux_Advanced_Customizer_Section' ) ) {
                    if ( ! class_exists( 'Redux_Customizer_Section' ) ) {
                        include_once ReduxFramework::$_dir . 'inc/extensions/customizer/inc/customizer_section.php';
                    }
                    include_once dirname( __FILE__ ) . '/inc/advanced_customizer_section.php';
                    if ( method_exists( $wp_customize, 'register_section_type' ) ) {
                        $wp_customize->register_section_type( 'Redux_Advanced_Customizer_Section' );
                    }
                }
                if ( ! class_exists( 'Redux_Advanced_Customizer_Panel' ) ) {
                    if ( ! class_exists( 'Redux_Customizer_Panel' ) ) {
                        include_once ReduxFramework::$_dir . 'inc/extensions/customizer/inc/customizer_panel.php';
                    }
                    include_once dirname( __FILE__ ) . '/inc/advanced_customizer_panel.php';
                    if ( method_exists( $wp_customize, 'register_panel_type' ) ) {
                        $wp_customize->register_panel_type( 'Redux_Advanced_Customizer_Panel' );
                    }
                }
                ...
                if ( ! class_exists( 'Redux_Advanced_Customizer_Section' ) ) {
                    include_once dirname( __FILE__ ) . '/advanced_customizer_section.php';
                    if ( method_exists( $wp_customize, 'register_section_type' ) ) {
                        $wp_customize->register_section_type( 'Redux_Advanced_Customizer_Section' );
                    }
                }
                if ( ! class_exists( 'Redux_Advanced_Customizer_Panel' ) ) {
                    include_once dirname( __FILE__ ) . '/advanced_customizer_panel.php';
                    if ( method_exists( $wp_customize, 'register_panel_type' ) ) {
                        $wp_customize->register_panel_type( 'Redux_Advanced_Customizer_Panel' );
                    }
                }
                if ( ! class_exists( 'Redux_Advanced_Customizer_Control' ) ) {
                    // NOT NEEDED FOR ADVANCED
                    //include_once( dirname( __FILE__ ) . '/advanced_customizer_control.php' );
                }
                ...
                //    $section = new Redux_Advanced_Customizer_rAds( $wp_customize, 'redux_rAds', array(
                //        'priority'    => 0,
                //    ) );
                ...
                //    ) );
                //
                //    //$wp_customize->add_control( new Redux_Advanced_Customizer_Control_rAds( $wp_customize, 'reduxAdsDisplay', array(
                //    //    'section'        => 'redux_rAds',
                ...
                if ( $this->parent->args['dev_mode'] ) {
                    ...
                    //    'priority'    => 0,
                    //);
                    ////$section = new Redux_Advanced_Customizer_Section( $wp_customize, 'redux_rAds', $args );
                    ...
                    //$section = new Redux_Advanced_Customizer_section_rAds( $wp_customize, 'redux_rAds', array(
                    //    'priority'    => -999,
                    //) );
                    //$wp_customize->add_section( $section, array(
                    //    'priority'    => -999,
                    ...
                    //    new Redux_Advanced_Customizer_Control_rAds(
                    //        $wp_customize,
                    //        'redux_rAds_empty',
                    //        array(
                    //            'section'    => 'redux_rAds',
                ...
                foreach ( $this->parent->sections as $key => $section ) {
                    ...
                    foreach ( $section['fields'] as $skey => $option ) {
                        ...
                        $class_name = 'Redux_Advanced_Customizer_Control_' . $option['type'];

                        do_action( 'redux/extension/customizer/control_init', $option['type'], $this->parent );

                        if ( ! class_exists( $class_name ) ) {
                            if ( ! file_exists( $this->upload_dir . $option['type'] . '.php' ) ) {
                                $template = str_replace( '{{type}}', $option['type'], '<?php' . PHP_EOL . '   class Redux_Advanced_Customizer_Control_{{type}} extends Redux_Advanced_Customizer_Control {' . PHP_EOL . '     public $type = "redux-{{type}}";' . PHP_EOL . '   }' );
                                $this->parent->filesystem->execute( 'put_contents', $this->upload_dir . $option['type'] . '.php', array( 'content' => $template ) );
                            }
                            include_once( $this->upload_dir . $option['type'] . '.php' );
                        }
            ...
            public function add_section( $id, $args = array(), $wp_customize ) {
                ...
                if ( is_a( $id, 'WP_Customize_Section' ) ) {
                    ...
                    $section = new Redux_Advanced_Customizer_Section( $wp_customize, $id, $args );
                }
            ...
            public function add_panel( $id, $args = array(), $wp_customize ) {
                if ( is_a( $id, 'WP_Customize_Panel' ) ) {
                    ...
                    $panel = new Redux_Advanced_Customizer_Panel( $wp_customize, $id, $args );
                }
        ...
        function redux_advanced_customizer_custom_validation( $field ) {
            return $field;
        }
    } // if
