...
<form class="variations_form cart" method="post" enctype='multipart/form-data' data-product_id="<?php echo absint( $product->id ); ?>" data-product_variations="<?php echo htmlspecialchars( json_encode( $available_variations ) ) ?>">
	...
	<?php else : ?>	<table class="variations" cellspacing="0">
		<tbody>
			<?php foreach ( $attributes as $attribute_name => $options ) : ?>
				<tr>
					<?php if(isset($pinnacle['product_radio']) && $pinnacle['product_radio'] == 1) { ?>
						<td class="product_label"><label for="<?php echo sanitize_title($attribute_name); ?>"><?php echo wc_attribute_label( $attribute_name ); ?> <span class="kad_required" title="required">*</span></label></td>
						 <td class="value">
						 	...
						 	echo end( $attribute_keys ) === $attribute_name ? '<a class="reset_variations" style="visibility: hidden;" href="#">' . __( 'Clear selection', 'pinnacle' ) . '</a>' : '';
						 ?>
						 </td>
