    ...
    class ReduxFramework_kad_slides {
        ...
        public function render() {
             ...
             $defaults = array(
                ...
                'content_title' => __ ( 'Slide', 'pinnacle' )
            );
           ...
           echo '<div class="redux-slides-accordion" data-new-content-title="' . esc_attr ( sprintf ( __ ( 'New %s', 'pinnacle' ), $this->field[ 'content_title' ] ) ) . '">';
            ...
            if ( isset ( $this->value ) && is_array ( $this->value ) && !empty ( $this->value ) ) {
                ...
                foreach ( $slides as $slide ) {
                   ...
                   if ( empty ( $slide[ 'thumb' ] ) && !empty ( $slide[ 'attachment_id' ] ) ) {
                    ...
                    echo '<span class="button media_upload_button" id="add_' . esc_attr($x) . '">' . __ ( 'Upload', 'pinnacle' ) . '</span>';
                    ...
                    echo '<li><input type="text" id="' . esc_attr($this->field['id']) . '-url_' . esc_attr($x) . '" name="' . esc_attr($this->field['name']) . '[' . esc_attr($x) . '][url]" value="' . esc_attr($slide['url']) . '" class="full-text upload" placeholder="'.__('URL', 'pinnacle').'" /></li>';
                    echo '<li><input type="text" id="' . esc_attr($this->field['id']) . '-title_' . esc_attr($x) . '" name="' . esc_attr($this->field['name']) . '[' . esc_attr($x) . '][title]" value="' . esc_attr($slide['title']) . '" placeholder="'.__('Title', 'pinnacle').'" class="full-text slide-title" /></li>';
                    ...
                    echo '<li><input type="text" id="' . esc_attr($this->field['id']) . '-link_' . esc_attr($x) . '" name="' . esc_attr($this->field['name']) . '[' . esc_attr($x) . '][link]" value="' . esc_attr($slide['link']) . '" placeholder="'.__('Slide Link', 'pinnacle').'" class="full-text" /></li>';
                    ...
                    echo '<li><a href="javascript:void(0);" class="button deletion redux-slides-remove">' . __('Delete Slide', 'pinnacle') . '</a></li>';
            ...
            if ($x == 0) {
                ...
                $placeholder = (isset($this->field['placeholder']['link'])) ? esc_attr($this->field['placeholder']['link']) : __( 'Slide Link', 'pinnacle' );
                ...
                echo '<input type="checkbox" class="checkbox-slide-target" id="' . esc_attr($this->field['id']) . '-target_' . esc_attr($x) . '" value="" ' . checked(  '', '1', false ) . ' name="' . $this->args['opt_name'] . '[' . esc_attr($this->field['id']) . '][' . esc_attr($x) . '][target]" />';
                echo ' '.__('Open Link in New Tab/Window', 'pinnacle'). '</label></li>';
            ...
            echo '</div><a href="javascript:void(0);" class="button redux-slides-add2 kad_redux-slides-add button-primary" rel-id="' . esc_attr($this->field[ 'id' ]) . '-ul" rel-name="' . $this->field[ 'name' ] . '[title][]">' . sprintf ( __ ( 'Add %s', 'pinnacle' ), $this->field[ 'content_title' ] ) . '</a><br/>';
        }
         ...
         public function enqueue () {
           ...
           wp_enqueue_script (
                'kad-field-slides-js', 
                get_template_directory_uri() . '/themeoptions/options_assets/extensions/kad_slides/kad_slides/field_kad_slides' . Redux_Functions::isMin () . '.js', 
            ...
            wp_enqueue_style (
                ...
                get_template_directory_uri() . '/themeoptions/options_assets/extensions/kad_slides/kad_slides/field_kad_slides.css', 
                time (), 
                true
            );
        }       
