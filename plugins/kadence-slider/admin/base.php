		...
		<div class="ksp_table_content ksp_table_row ksp-clearfix">
				...
				if(!empty($slides)) {
			       if(isset($slides[0]->background_type_image) && !empty($slides[0]->background_type_image) && $slides[0]->background_type_image != 'none') {
			       	 ...
			       	 echo '<div class="kt_placement_sliders" style="background:'.$slides[0]->background_type_color.'"></div>';
			       } else {
				...
				<div class="ksp-table-column ksp_column_03"><a href="<?php echo '?page=kadenceslider&view=layeredit&id=' . $slider->id . '"';?>"><?php echo $slider->name?></a></div>
				<div class="ksp-table-column ksp_column_04">[kadence_slider_pro id="<?php echo $slider->id; ?>"]</div>
				<div class="ksp-table-column ksp_column_05">
					...
					<a class="ksp-duplicate-slider ksp-button ksp-is-warning" href="javascript:void(0)" data-duplicate="<?php echo $slider->id; ?>"><?php echo __('Duplicate Slider', 'kadence-slider');?></a>
					<a class="ksp-export-slider ksp-button ksp-is-warning" href="javascript:void(0)" data-export="<?php echo $slider->id; ?>"><?php echo __('Export Slider', 'kadence-slider');?></a>
					<a class="ksp-delete-slider ksp-button ksp-is-danger" href="javascript:void(0)" data-delete="<?php echo $slider->id; ?>"><?php echo __('Delete Slider', 'kadence-slider');?></a>
				</div>
				</div>
				<?php }
			} else { ?>
	...
	while ( $wp_query->have_posts() ) : $wp_query->the_post();
		...
		<div class="ksp_table_content ksp_table_row ksp-clearfix">
				...
				if(!empty($slides[0]['image_id'])) {
					$image = wp_get_attachment_image( $slides[0]['image_id'], 'thumbnail' );
					echo $image;
				} else {
				}?>
				</div>
				<div class="ksp-table-column ksp_column_02"><?php echo $post->ID ;?></div>
